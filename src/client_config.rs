...
use crate::{Certificate, Error, Identity, TlsConnector};
...
impl ClientTlsConfig {
    ...
    pub fn tls_connector(&self, uri: Uri) -> Result<TlsConnector, Error> {
        let domain = match &self.domain {
            None => uri.host().ok_or(Error::InvalidUri)?.to_string(),
